import unittest
from core.adjudicator.territory import SeaTerritory

from core.adjudicator.tests.datc import test_data
from core.adjudicator.tests.datc.test_data import territories as T
from core.adjudicator.tests.datc.test_data import nations as n
from core.adjudicator.decisions import ILLEGAL, LEGAL, NO_PATH
from core.adjudicator.piece import Army, Fleet
from core.adjudicator.processor import process_orders
...
class Territories:
    IRISH_SEA = T['irish sea']
    KIEL = T['kiel']
    LIVERPOOL = T['liverpool']
    LONDON = T['london']
    MUNICH = T['munich']
    NORTH_SEA = T['north sea']
    PICARDY = T['picardy']
...
class TestBasicChecks(unittest.TestCase):
    ...
    def test_moving_to_an_area_that_is_not_a_neighbour(self):
        ...
        self.assertEqual(order.legal_decision(), ILLEGAL)
        self.assertEqual(order.legal_decision.error_code, 'M002')
    ...
    def test_move_army_to_sea(self):
        ...
        self.assertEqual(order.legal_decision(), ILLEGAL)
        self.assertEqual(order.legal_decision.error_code, 'M003')
    ...
    def test_ordering_a_unit_of_another_country(self):
        ...
        self.assertEqual(order.legal_decision(), ILLEGAL)
        self.assertEqual(order.legal_decision.error_code, 'M001')
